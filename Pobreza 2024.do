...
gl log="\\tsclient\C\Users\Juan Alvaro\OneDrive - up.edu.mx\Notes\Politicas de disminución de pobreza\STATA_PM_2024\Log"

log using "$log\Pobreza_24.txt", text replace

*********************************************************
*
* INEGI. POBREZA MULTIDIMENSIONAL. PROGRAMA DE CÁCULO 2024
*
*********************************************************
...
*Año de nacimiento
gen anac_e=.
replace anac_e=2024-edad if edad!=.
...
label define niv_ed 0 "Con primaria incompleta o menos" 1 "Primaria completa o secundaria incompleta" 2 "Secundaria completa o media superior incompleta" 3 "Media superior completa o mayor nivel educativo"
...
sort folioviv foliohog numren
save "$bases\ocupados24.dta", replace
...
sort folioviv foliohog numren
merge folioviv foliohog numren using "$bases\ocupados24.dta"
tab _merge
drop _merge
...
label define tipo_trab 1 "Depende de un patrón, jefe o superior" 2 "No depende de un jefe y recibe o tiene asignado un sueldo" 3 "No depende de un jefe y no recibe o no tiene asignado un sueldo"
...
*Ocupación principal
gen smlab1=.
replace smlab1=1 if ocupa1==1 & (inst_1=="1" | inst_2=="2" | inst_3=="3" | inst_4=="4") & (inscr_1=="1")
recode smlab1 (.=0) if ocupa1==1

label var smlab1 "Servicios médicos por prestación laboral en ocupación principal"
label define smlab 0 "Sin servicios médicos" 1 "Con servicios médicos"
label value smlab1 smlab

*Ocupación secundaria
gen smlab2=.
replace smlab2=1 if ocupa2==1 & (inst_1=="1" | inst_2=="2" | inst_3=="3" | inst_4=="4") & (inscr_1=="1")
...
gen smcv=.
replace smcv=1 if (inst_1=="1" | inst_2=="2" | inst_3=="3" | inst_4=="4") & (inscr_6=="6") & (edad>=12 & edad!=.)
...
label define sa_dir 0 "Sin acceso" 1 "Con acceso"
...
label define par 1 "Jefe o jefa del hogar" 2 "Cónyuge del jefe/a" 3 "Hijo del jefe/a" 4 "Padre o Madre del jefe/a" 5 "Suegro del jefe/a" 6 "Sin parentesco directo"
label value par par
...
*Acceso reportado
replace ic_asalud=0 if (inst_1=="1" | inst_2=="2" | inst_3=="3" | inst_4=="4" | inst_5=="5" | inst_6=="6" | inst_7=="7"| inst_8=="8")
recode ic_asalud .=1
...
replace discap=0 if (disc_oir=="1" | disc_oir=="2")
...
replace discap=1 if disc_oir=="3" | disc_oir=="4"
replace discap=1 if disc_vest=="3" | disc_vest=="4"
replace discap=1 if disc_habla=="3" | disc_habla=="4"
replace discap=1 if disc_acti=="3" | disc_acti=="4"

label var discap "Población según condición de discapacidad física o mental"
label define discap 0 "Sin discapacidad" 1 "Con discapacidad"
label value discap discap

keep folioviv foliohog numren sexo sa_* *_sa inst_* inscr_* segvol_* ic_asalud discap
sort folioviv foliohog numren
save "$bases\ic_asalud24.dta", replace
...
sort folioviv foliohog numren
save "$bases\prestaciones24.dta", replace
...
scalar	feb24 =	0.9828545801
scalar	mar24 =	0.9856778396
scalar	abr24 =	0.9876702962
scalar	may24 =	0.9858395889
scalar	jun24 =	0.9895671737
scalar	jul24 =	0.9999264776
scalar	ago24 =	1.0000000000
scalar	sep24 =	1.0004926000
scalar	oct24 =	1.0059920743
scalar	nov24 =	1.0103740084
scalar	dic24 =	1.0142339335
...
egen double ing_pens=rmean(ing_1 ing_2 ing_3 ing_4 ing_5 ing_6) if clave=="P032" | clave=="P033"
egen double ing_pam=rmean(ing_1 ing_2 ing_3 ing_4 ing_5 ing_6) if clave=="P104" | clave=="P045"
...
sort folioviv foliohog numren
save "$bases\pensiones24.dta", replace
...
sort folioviv foliohog numren
merge folioviv foliohog numren using "$bases\prestaciones24.dta"
tab _merge
drop _merge

sort folioviv foliohog numren
merge folioviv foliohog numren using "$bases\pensiones24.dta"
tab _merge
drop _merge
...
label define jub 0 "Población no pensionada o jubilada" 1 "Población pensionada o jubilada"
label value jub jub
...
*Contratación voluntaria: servicios médicos y ahorro para el retiro o pensión para la vejez (SAR o Afore)
...
label define ss_dir 0 "Sin acceso" 1 "Con acceso"
...
replace hijo=. if par==3 & ss_dir==1 & (((inst_2=="2" | inst_3=="3") & inscr_6=="6") & (inst_1=="" & inst_4=="" & inst_8=="") & (inscr_1=="" & inscr_2=="" & inscr_3=="" & inscr_4=="" & inscr_5=="" & inscr_7==""))
...
*Otros núcleos familiares: se identifica a la población con acceso a la seguridad social mediante otros núcleos familiares a través de la afiliación o inscripción a servicios de salud por algún familiar dentro o fuera del hogar, muerte del asegurado o por contratación propia
...
destring agua_ent, gen(isb_agua)
recode isb_agua (0=.)
recode isb_agua (1 2=0) (3=1)
...
*Combustible
rename combus combustible, replace
destring combustible, gen(combus) force
destring fogon_chi, gen(estufa)
recode combus (0=.) (-1=.)
recode estufa (-1=.)
gen isb_combus=0 if combus>=3 & combus<=6
replace isb_combus=0 if combus==7
...
save "$bases\ic_sbv24.dta", replace
...
save "$bases\menores24.dta", replace
...
sort folioviv foliohog
merge folioviv foliohog using "$bases\menores24.dta"
tab _merge
drop _merge
...
save "$bases\ic_ali24.dta", replace
...
sort folioviv foliohog numren
save "$bases\aguinaldo24.dta", replace
...
merge folioviv foliohog numren using "$bases\aguinaldo24.dta"
tab _merge
drop _merge
...
gen double sal_nm=gasnomon if (clave>="121201" & clave<="121202") | clave=="131209" | clave=="133091" | (clave>="061111" & clave<="061119") | (clave>="06111A" & clave<="06111L") | clave=="061120" | (clave>="061211" & clave<="061212") | (clave>="061221" & clave<="061223") | clave=="061230" | (clave>="061311" & clave<="061312") | clave=="061320" | (clave>="061331" & clave<="061336") | (clave>="061401" & clave<="061402") | clave=="062110" | (clave>="062191" & clave<="062198") | clave=="062210" | clave=="062291" | clave=="062292" | (clave>="062311" & clave<="062313") | (clave>="062321" & clave<="062323") | (clave>="063101" & clave<="063106") | clave=="063200" | (clave>="064101" & clave<="064103") | (clave>="064201" & clave<="064202")
...
gen double cuip_nm=gasnomon if (clave>="131111" & clave<="131113") | clave=="131120" | (clave>="131201" & clave<="131208") | (clave>="131311" & clave<="131313") | (clave>="131321" & clave<="131323") | clave=="132916" | clave=="139010" | clave=="061338" | (clave>="13120A" & clave<="13120N")
...
sort folioviv foliohog
save "$bases\esp_def24.dta", replace
use "$bases\ingresonomonetario_def24.dta", clear
...
sort folioviv foliohog
save "$bases\reg_def24.dta", replace
...
merge folioviv foliohog using "$bases\ingreso_deflactado24.dta"
tab _merge
drop _merge
...
merge folioviv foliohog using "$bases\esp_def24.dta"
tab _merge
drop _merge
...
merge folioviv foliohog using "$bases\reg_def24.dta"
tab _merge
drop _merge
...
sort folioviv foliohog
save "$bases\ingresotot24.dta", replace
...
sort folioviv foliohog
save "$bases\tamhogesc24.dta", replace
...
use "$bases\ingresotot24.dta", clear

*Incorporación de la información sobre el tamaño del hogar ajustado

merge folioviv foliohog using "$bases\tamhogesc24.dta"
tab _merge
drop _merge
...
scalar lp2_urb = 4564.97
scalar lp2_rur = 3296.92
...
merge folioviv foliohog numren using "$bases\ic_asalud24.dta"
tab _merge
drop _merge
sort folioviv foliohog numren

merge folioviv foliohog numren using "$bases\ic_segsoc24.dta"
tab _merge
drop _merge
sort folioviv foliohog

merge folioviv foliohog using "$bases\ic_cev24.dta"
tab _merge
drop _merge
sort folioviv foliohog

merge folioviv foliohog using "$bases\ic_sbv24.dta"
tab _merge
drop _merge
sort folioviv foliohog

merge folioviv foliohog using "$bases\ic_ali24.dta"
tab _merge
drop _merge
sort folioviv foliohog

merge folioviv foliohog using "$bases\p_ingresos24.dta"
tab _merge
drop _merge
...
*Distancia normalizada del ingreso respecto a las líneas de pobreza por ingresos
...
label var prof1 "Índice FGT con alfa igual a 1 (líneas de pobreza por ingresos)"

*Distancia normalizada del ingreso respecto a las líneas de pobreza extrema por ingresos
...
label var prof_e1 "Índice FGT con alfa igual a 1 (líneas de pobreza extrema por ingresos)"
...
gen double profun=i_privacion/6
...
gen double int_pob=profun*pobreza
...
gen double int_pobe=profun*pobreza_e
...
gen double int_vulcar=profun*vul_car
...
gen double int_caren=profun*carencias
...
sort folioviv foliohog numren
save "$bases\pobreza_24.dta", replace
